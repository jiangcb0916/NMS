...
- Git 本地操作和远端同步要分开处理。除非用户明确要求 `push`、上传代码、同步 GitHub 或服务器部署，不要自动执行 `git push`、远端同步或部署动作。
...
- 用户明确要求上传代码、推送或同步远端时，默认同时推送两份：内网仓库 `qschou`（`git@code.qschou.com:13193400916/nms.git`）和 GitHub 仓库 `origin`。内网仓库 SSH 已配置好时直接用 `git push qschou main`，GitHub 使用 `git push origin main`。
